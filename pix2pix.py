...
if a.gpu_id != '0':
    ...
    os.environ["CUDA_VISIBLE_DEVICES"] = a.gpu_id  # specify which GPU(s) to be used

print("{}-channel images".format(a.channel))
if a.mask:
    print("calculate loss only based on masked part")
else:
...
def check_image(image):
    assertion = tf.assert_greater_equal(tf.shape(image)[-1], 3, message="image must have at least 3 color channels")
...
def rgb_to_lab(srgb):
    with tf.name_scope("rgb_to_lab"):
        ...
        # srgb_pixels = tf.reshape(srgb, [-1, tf.shape(srgb)[-1]])
...
def load_examples():
    ...
    with tf.name_scope("load_images"):
        ...
        assertion = tf.assert_greater_equal(tf.shape(raw_input)[2], 3, message="image does not have 3 or 4 channels")
        with tf.control_dependencies([assertion]):
            raw_input = tf.identity(raw_input)
        raw_input.set_shape([None, None, a.channel])
...
def main():
    ...
    if a.mode == "export":
        ...
        input_image.set_shape([CROP_SIZE, CROP_SIZE, a.channel])
        batch_input = tf.expand_dims(input_image, axis=0)

        with tf.variable_scope("generator"):
            batch_output = deprocess(create_generator(preprocess(batch_input), tf.shape(input_image)[2]))
